...
#Реализация алгоритма AES_CBC на основе AES_ECB
def AES_CBC(text: bytes, key: bytes, block_size: int=16) -> bytes:
    text = [text[i * block_size: (i + 1) * block_size] for i in range(len(text) // block_size)]
    cipher = AES.new(key, AES.MODE_ECB)
    IV = bytes([0]) * 16
    ans = b''
    for i in text:
        res = xor(IV, cipher.decrypt(i))
        IV = i
        ans += res
    return ans
